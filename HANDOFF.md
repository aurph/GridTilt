...
## When the user says "make it more X"
- "more professional / less marketed" → drop superlatives, lowercase voice, plain descriptive names (NPI not "Nuclear Renaissance"). The user has called out "thesis," "renaissance," "cringe" branded framings as the things to avoid.
- "more tangible" in tweet content → narrative line tied to real data (which sector dominated, which constituent led/lagged), not generic phrases.
- "more character" → light brand mark (the `⚡`), conversational narration ("uranium miners got sold today"), but no emoji spam.
